...
以 [Astro](https://astro.build) 實作的電子書工作室官網(旗下兩位作者),對應《電子書網站設計規格書》v0.2。
作品展示+導購(Apple Books / Google Play 圖書),無金流、會員、購物車;首頁不放購書連結,購書按鈕僅出現在書籍單頁。
...
1. `src/config.ts`:工作室名稱、兩位作者筆名(`AUTHORS`,改名後書籍檔的 `author` 欄位要同步改)、網站描述、著作權起始年、Email(拆成帳號/網域兩欄)、Instagram 與 Facebook 網址、兩平台作者頁連結。
2. 刪除範例書籍/消息/文章與示意圖,換上真實內容(範例內文都標有「範例」字樣);每本書的 `author` 填真實筆名。
...
5. `src/pages/about.astro`:填工作室介紹與兩位作者簡介;`contact.astro` 填可洽談事項。
...
書籍欄位對應規格書 6.1 節;`category` 只能是 `繪本`/`童話`/`少年小說`/`新詩`,繪本與童話必填 `age_range`;`author` 必填且必須是 `src/config.ts` 裡 `AUTHORS` 列出的筆名之一(合著時可填多位,寫成 YAML 清單)。漏填或填錯時建置會直接報錯提醒。
